    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify({
                "fulfillment_response": {
                    "messages": [
                        {"text": {"text": [f"Hey {order_details["name"]}, you have ordered {order_data["quantity"]}{order_data["items"]}(s), please update your order accordingl"]}}
                    ]
                }
            })
    ...
    try:
        ...
        if response.status_code == 200:
            return jsonify({
                "fulfillment_response": {
                    "messages": [
                        {"text": {"text": [f"Your order has been successfully updated."]}}
                    ]
                }
            })
        else:
